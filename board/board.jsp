	...
	<body>
		<div id="main">
			...
			<section>
				<div class="board">
					<div>
						<img src="../img/board/board_title.jpg">
					</div>
					<form name="user" method="POST" action="add.jsp">
						<div class="board_input">
							<div>
								<label>姓名</label>
								<input type="text" name="name"  value="莊亞琦">
							</div>
							<div >
								<label >留言</label>
					...
					try{
						...
						try{
							if(con.isClosed())
								...
								sql="select * from guestbook"; //算出共幾筆資料
								ResultSet rs=con.createStatement().executeQuery(sql);
								//先移到檔尾, getRow()後, 就可知道共有幾筆記錄
								rs.last();
								int total_content=rs.getRow();
								//每頁顯示5筆, 算出共幾頁
								int page_num=(int)Math.ceil((double)total_content/3); //無條件進位
								...
								//Step 5: 顯示結果
								int start_record=(int)Math.ceil((double)(current_page-1)*3);
								//遞減排序, 讓最新的資料排在最前面
					...
					<%while(rs.next()){%>
						<div class="view_message">
							...
							<div class="message_div">
								...
								<br>
								<span class="time">
									<%out.println(rs.getString("putdate"));%>
								</span>
							</div>
							<div class="message_div">
								<label>留言內容	</label>
...
<!-- 						<div class="view_message">
							...
							<div class="message_div">
								...
								<span class="text">
									<textarea readonly="readonly">
									disabled 属性disabled 属性disabled 属性disabled 属性disabled 属性disabled 属性
									disabled 属性disabled 属性disabled 属性disabled 属性disabled 属性disabled 属性
									disabled 属性disabled 属性disabled 属性disabled 属性disabled 属性disabled 属性
									disabled 属性disabled 属性disabled 属性disabled 属性disabled 属性disabled 属性
									</textarea>
					...
					<%
						...
						int total_content2=rs2.getRow();  //先移到檔尾, getRow()後, 就可知道共有幾筆記錄
						//每頁顯示9筆, 算出共幾頁
						int page_num2=(int)Math.ceil((double)total_content2/3.0); //無條件進位
						if (page_num2==0) //無留言時將頁數指標訂為1
						page_num2=1;
						out.println("<div class='page'>");
						for(int i=1;i<=page_num2;i++) //使用get傳輸方式,建立1,2,...頁超連結
						{
							out.print("<a href='board.jsp?page="+i+"'>"+i+"</a>&nbsp;"); //&nbsp;html的空白
						}
						out.println("</div>");
						}
				}
