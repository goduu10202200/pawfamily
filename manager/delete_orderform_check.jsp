...
//Step 4: 執行 SQL 指令	
            sql = "delete from order_form where no='"+delete_no+"'";
			int no = con.createStatement().executeUpdate(sql);
%>

<%			
			if(no>0)
				out.println("<script language='javascript'>alert(\"刪除成功 !!\");window.location.href='order_view_manager.jsp';</script>");
			else if(request.getParameter("dele")=="")
				out.println("<script language='javascript'>alert(\"你尚未輸入欲刪除之訂購單編號 !!\");window.location.href='order_view_manager.jsp';</script>");
			else
