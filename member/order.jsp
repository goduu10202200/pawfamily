					...
					try{
						...
						try{
							if(con.isClosed())
								...
								sql="select * from order_form where id='" +session.getAttribute("id")+"'"; //算出共幾筆資料
								...
								//遞減排序, 讓最新的資料排在最前面
								sql="select * from order_form where id='" +session.getAttribute("id")+"' ORDER BY no DESC LIMIT "; //LIMIT是限制傳回筆數
								//sql+=start_record;
								sql+=start_record+",5";
								//上述語法解讀如下:
				...
				%>
							<div class="orderinfo">
								<table width="100%" border="0">
									<thead>
										<tr>
											<th class="order_date">訂貨日期</th>
											<th class="order_id">訂單編號</th>
											<th class="order_paying">付款方式</th>
											<th class="order_total">總價</th>
											<th class="order_delete">取消訂單</th>
										</tr>
									</thead>
									<% while(rs.next()){
										out.println("<tbody>");
											out.println("<tr>");
												...
												////
												out.println("<td>");
													out.println("<form id='order_id' method='post' action='order_content.jsp'>");
														out.println("<input type='hidden' name='order_id'value='"+rs.getString("order_id")+"'>");
														//out.println("String  order_id = rs.getString('order_id')");
														//out.println("session.setAttribute('order_id', order_id);");
														out.println("<a  href='javascript:;' onclick='parentNode.submit();'>");
												...
												out.println("<td>");
													out.println("<span>");
														out.println(rs.getString("pay_way"));
													out.println("</span>");
												out.println("</td>");
												////
												out.println("<td>");
									...
									%>
												<td>
													<form name="checkbox" method="post" action="order_delete.jsp">
													<input type="hidden" name="delete" value="<%=rs.getString("order_id")%>">
														<button type="submit" class="btn  btn-lg Paw_button" 
														style="background-color: #f9f9f9; outline-color:#f9f9f9;">
									...
									<!-- <tbody>
										<tr>
											...
											<td>777</td>
											<td>
												<button type="button" class="btn  btn-lg Paw_button" style="background-color: #f9f9f9; outline-color:#f9f9f9;">
												<span class="glyphicon glyphicon-trash" aria-hidden="true"></span>
												</button>
											</td>
										</tr>
								...
								<%
									sql="select * from order_form where id='"+session.getAttribute("id")+"'";
									ResultSet rs2=con.createStatement().executeQuery(sql);
								%>
								<%
									rs2.last();
									...
									out.println("<div class='page'>");
										...
										{
											out.print("<a href='order.jsp?page="+i+"'>"+i+"</a>&nbsp;"); //&nbsp;html的空白
										}
									out.println("</div>");
										}
									}
